...
- 新增 `npm run typecheck` 与 `npm run lint:all` 质量检查脚本，便于发版前统一执行静态校验。
- 为 `ws`、`cmdk` 与 `react-resizable-panels` 增加本地类型声明，消除当前工程内的 TypeScript 阻塞。

### 修复

- 修复主进程日志器、错误处理、模块解析、Windows 兼容检查与启动优化中的多处类型和运行时脆弱点。
- 修复网关请求错误记录与健康检查调用逻辑，减少主进程运行期误报。
- 修复 About 菜单版本展示与未处理 Promise 拒绝日志记录，确保发版后的应用信息与异常日志更准确。

### 变更

- 发布桌面端正式版本 `v1.1.0`，统一 README、下载链接和开发说明。
