; installer.nsh — 喵哥Claw 自定义 NSIS 安装钩子
...
!macro customInstall
  ...
  nsExec::ExecToStack 'netsh advfirewall firewall delete rule name="喵哥Claw"'
  Pop $0
  DetailPrint "添加入站规则（TCP 18789）..."
  nsExec::ExecToStack 'netsh advfirewall firewall add rule name="喵哥Claw" dir=in action=allow program="$INSTDIR\喵哥Claw Desktop.exe" enable=yes'
  Pop $0
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "是否同时删除 喵哥Claw 的用户设置和缓存数据？$\n$\n\
选择「是」将彻底清除（AppData\Roaming\喵哥Claw Desktop），$\n\
选择「否」则保留，重新安装后可自动恢复设置。" \
    IDNO done
  RMDir /r "$APPDATA\喵哥Claw Desktop"
  DetailPrint "用户数据已清除。"
  done:
!macroend
